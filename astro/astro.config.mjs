...
function remarkCustomImage() {
  return (tree) => {
    visit(tree, 'image', (node) => {
      ...
      // Replace the original image node with an HTML node
      node.type = 'html';
      node.value = html;
      delete node.url;
      delete node.alt;
...
function customImagePlugin() {
  return (tree) => {
    visit(tree, 'image', (node) => {
      ...
      // Replace the original image node with our new figure node
...
function remarkImagePlugin() {
  return (tree, file) => {
    visit(tree, 'image', (node) => {
      ...
      return h('img', { src: 'node.url', alt: node.alt, class: 'custom-image' });
      ...
      //   type: 'html',
      //   value: `123`,
      //   // value: `<img src="${url}" alt="${alt}" class="custom-image" />`,
      // };
    });
  };
}
